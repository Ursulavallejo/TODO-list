...
const NavBar = () => {
    ...
    return <nav className={css.navbarLayout}>
        <ul className={css.navBarList}>
            ...
            {loggedIn ? <li className={css.navBarItem}>
                    <Link className={css.navbarLink} to={RoutingPath.signedInView}>My Todo
                        List</Link></li> :
                <li data-testid='logIn' className={css.navBarItem}>
                    <Link className={css.navbarLink}
                          to={RoutingPath.signInView}>Log In</Link></li>}
        </ul>
    </nav>
};
